...
def Neural_Sim(self, input, output):
    ...
    if len(self.weight.shape) > 2:
        ...
        stride = self.stride
        activity = write_matrix_activation_conv(stretch_input(input[0].cpu(
        ).data.numpy(), k, padding, stride), None, self.wl_input, input_file_name)
        input_activity.write(str(activity) + ",")
    else:
        activity = write_matrix_activation_fc(
            input[0].cpu().data.numpy(), None, self.wl_input, input_file_name)
        if (str(self.name) == 'FC2_'):
            input_activity.write(str(activity) + "\n")
        else:
            input_activity.write(str(activity) + ",")
    f.write(weight_file_name + ' ' + weightOld_file_name +
            ' ' + input_file_name + ' ' + str(activity) + ' ')
...
def write_matrix_activation_conv(input_matrix, fill_dimension, length, filename):
    filled_matrix_b = np.zeros(
        [input_matrix.shape[2], input_matrix.shape[1] * length], dtype=np.str)
    ...
    activity = np.sum(filled_matrix_b.astype(np.float),
                      axis=None) / np.size(filled_matrix_b)
...
def stretch_input(input_matrix, window_size=5, padding=(0, 0), stride=(1, 1)):
    ...
    output_matrix = np.zeros((input_shape[0], int(
        item_num), input_shape[1] * window_size * window_size))
    iter = 0
    for i in range(input_shape[2] - window_size + 1):
        for j in range(input_shape[3] - window_size + 1):
            for b in range(input_shape[0]):
                output_matrix[b, iter, :] = input_matrix[b, :, i:i + window_size,
                                                         j: j + window_size].reshape(input_shape[1] * window_size * window_size)
            iter += 1
...
def hardware_evaluation(model, wl_weight, wl_activation, numEpoch):
    ...
    f.write('./NeuroSIM/main ' + str(numEpoch) + ' ./NeuroSIM/NetWork.csv ' +
            str(wl_weight) + ' ' + str(wl_activation) + ' ')
