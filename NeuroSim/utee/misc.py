...
def auto_select_gpu(mem_bound=500, utility_bound=0, gpus=(0, 1, 2, 3, 4, 5, 6, 7), num_gpu=1, selected_gpus=None):
    ...
    if selected_gpus is None:
        ...
        for i in range(5):  # sample 5 times
            ...
            utility = [int(re.compile('\d+').findall(s)[0])
                       for s in re.compile('\d+%\s+Default').findall(info)]
        ...
        nGPU = len(utility)
        ideal_gpus = [i for i in range(
            nGPU) if mem[i] <= mem_bound and utility[i] <= utility_bound and i in gpus]

        if len(ideal_gpus) < num_gpu:
            print("No sufficient resource, available: {}, require {} gpu".format(
                ideal_gpus, num_gpu))
            sys.exit(0)
        else:
        ...
        with env.begin() as txn:
            ...
            for key, value in cursor:
                ...
                img = cv2.imdecode(np.fromstring(
                    value, np.uint8), cv2.IMREAD_COLOR)
