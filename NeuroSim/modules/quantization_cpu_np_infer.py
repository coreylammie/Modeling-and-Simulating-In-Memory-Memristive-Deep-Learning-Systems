...
class QConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size,
                 ...
                 wl_input=8, wl_activate=8, wl_error=8, wl_weight=8, inference=0, onoffratio=10, cellBit=1, subArray=128, ADCprecision=5, vari=0, t=0, v=0, detect=0, target=0, debug=0, name='Qconv', device=torch.device('cuda')):
    ...
    def forward(self, input):
                    ...
                    if numSubArray == 0:
                        ...
                        if weight.shape[1] == 3:
                            ...
                            X_decimal = torch.round(
                                (2**bitWeight - 1) / 2 * (weight + 1) + 0) * mask
                            ...
                            for k in range(int(bitWeight / self.cellBit)):
                                ...
                                # weight cannot map to 0, but to Gmin
                                remainderQ = (
                                    upper - lower) * (remainder - 0) + (cellRange - 1) * lower
                                remainderQ = remainderQ + \
                                    torch.normal(0., torch.full(
                                        remainderQ.size(), self.vari, device=self.device))
                                ...
                                outputDummyPartial = F.conv2d(
                                    input, dummyP * mask, self.bias, self.stride, self.padding, self.dilation, self.groups)
                                scaler = cellRange**k
                                outputP = outputP + outputPartial * \
                                    scaler * 2 / (1 - 1 / onoffratio)
                                ...
                                for k in range(int(bitWeight / self.cellBit)):
                                    ...
                                    remainderQ = remainderQ + remainderQ * \
                                        torch.normal(0., torch.full(
                                            remainderQ.size(), self.vari, device=self.device))
                                    outputPartial = F.conv2d(
                                        inputB, remainderQ * mask, self.bias, self.stride, self.padding, self.dilation, self.groups)
                                    outputDummyPartial = F.conv2d(
                                        inputB, dummyP * mask, self.bias, self.stride, self.padding, self.dilation, self.groups)
                                    ...
                                    scaler = cellRange**k
                                    outputP = outputP + outputPartialQ * \
                                        scaler * 2 / (1 - 1 / onoffratio)
                                    outputD = outputD + outputDummyPartialQ * \
                                        scaler * 2 / (1 - 1 / onoffratio)
                                scalerIN = 2**z
                                outputIN = outputIN + \
                                    (outputP - outputD) * scalerIN
                            ...
                            for s in range(numSubArray):
                                ...
                                for k in range(int(bitWeight / self.cellBit)):
                                    ...
                                    outputDummyPartialQ = wage_quantizer.LinearQuantizeOut(
                                        outputDummyPartial, self.ADCprecision)
                                    scaler = cellRange**k
                                    outputSP = outputSP + outputPartialQ * \
                                        scaler * 2 / (1 - 1 / onoffratio)
                                    outputD = outputD + outputDummyPartialQ * \
                                        scaler * 2 / (1 - 1 / onoffratio)
                                    if (weight.shape[0] == 256) & (weight.shape[1] == 128):
                                        weightMatrix = (
                                            remainderQ * mask).cpu().data.numpy()
                                        weight_file_name = './layer_record/weightForLayer3_subarray' + \
                                            str(s) + '_weightBitNo_' + \
                                            str(k) + ".csv"
                                        ...
                                        weight_matrix = weightMatrix.reshape(
                                            cout, -1).transpose()
                                        np.savetxt(
                                            weight_file_name, weight_matrix, delimiter=",", fmt='%10.5f')
        ...
        output = wage_quantizer.WAGEQuantizer_f(
            output, self.wl_activate, self.wl_error)
...
class QLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=False, logger=None, clip_weight=False, wage_init=False, quantize_weight=False, clip_output=False, quantize_output=False,
                 wl_input=8, wl_activate=8, wl_error=8, wl_weight=8, inference=0, onoffratio=10, cellBit=1, subArray=128, ADCprecision=5, vari=0, t=0, v=0, detect=0, target=0, debug=0, name='Qlinear', device=torch.device('cuda')):
